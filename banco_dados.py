...
def producao():
    ...
    # TO-DO: fazer o download do arquivo no site e, caso não encontre, processar o csv da pasta 'dados_embrapa'
    dados_embrapa_producao = pd.read_csv(diretorio + '/dados_embrapa/producao.csv', sep=';')
    ...
    dados_embrapa_producao['cod_produto'] = dados_embrapa_producao['cod_produto'].str.lower()
    ...
    dados_embrapa_producao['cod_produto'] = dados_embrapa_producao['cod_produto'].replace(to_replace=caracteres_substituir, regex=True)
    
    # Pivota os dados de produção, transformando os anos em 'observações'
    banco_dados_producao = dados_embrapa_producao.melt(id_vars=['cod_produto', 'produto'], var_name='ano', value_name='litros')
    # Salva os dados pivotados de produção em um novo CSV para ser posteriormente utilizado pela API
    banco_dados_producao.to_csv(diretorio + '/banco_dados/producao.csv', sep=';', index=False)
